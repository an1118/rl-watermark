...
para_score_strategy="dynamic"
para_growth_rate=0.4
...
if [ "$ori_score_strategy" = "abs" ]; then
  run_id=$(echo "$run_id" | sed "s/ori(${ori_score_strategy})/ori(${ori_score_strategy}-${target_ori_score})/")
fi
if [ "$ori_score_strategy" = "dynamic" ] || [ "$ori_score_strategy" = "gap" ]; then
  run_id=$(echo "$run_id" | sed "s/ori(${ori_score_strategy})/ori(${ori_score_strategy}-${ori_growth_rate})/")
fi
if [ "$wm_score_strategy" = "dynamic" ]; then
  run_id=$(echo "$run_id" | sed "s/wm(${wm_score_strategy})/wm(${wm_score_strategy}-${wm_growth_rate})/")
fi
if [ "$para_score_strategy" = "dynamic" ]; then
  run_id=$(echo "$run_id" | sed "s/para(${para_score_strategy})/para(${para_score_strategy}-${para_growth_rate})/")
fi
